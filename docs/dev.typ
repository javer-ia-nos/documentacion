...
2. El repositorio `integracion` contiene un `docker-compose.yml` que orquesta la infraestructura viva:
  - *Base de datos:* PostgreSQL 16 Alpine con healthcheck.
  - *Broker de eventos:* Apache Kafka en modo Kraft.
  - *Borde:* `api-gateway` (puerto `4860`).
  - *Microservicios:* Los 8 servicios del banco conectados por la red interna de Docker.
...
```
integracion/
├── docker-compose.yml           # Levanta Postgres, Kafka, Gateway y los 8 microservicios
